...
const processRecords = function(records){
  ...
  records.forEach(record => {
    ...
    if(NIGHTTIME_RANGE.includes(dispatched_time.getHours())){
...
const createStats = function(raw_values){
  ...
  raw_values.num_personnel_on_incidents.forEach((num, i) => {
    ...
    let incident = {
      sum:  sum(modified_incident_times),
      mean: mean(modified_incident_times)
      // median: median(modified_incident_times),
      // min: min(modified_incident_times),
      // max: max(modified_incident_times)
    };
    stats_output.personnel_stats['time_on_incident'] = incident;
  })

  stats_output.personnel_stats['number_per_incident'] = {
    mean: mean(raw_values.num_personnel_on_incidents),
    // median: median(raw_values.num_personnel_on_incidents),
    min: min(raw_values.num_personnel_on_incidents),
    max: max(raw_values.num_personnel_on_incidents)
  }

  // stats_output.apparatus_stats = {
  //   per_incident: {
  //     mean: mean(raw_values.num_apparatus_on_incidents),
  //     median: median(raw_values.num_apparatus_on_incidents),
  //     min: min(raw_values.num_apparatus_on_incidents),
  //     max: max(raw_values.num_apparatus_on_incidents)
  //   }
  // }
