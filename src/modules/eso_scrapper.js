...
const _processRecords = function (records, startDate, stopDate, dayRange) {
  ...
  const incidentsInDispatchedDateOrder = _.uniq(_.map(recordsInRange, 'Incident Number'))
  ...
  incidentsInDispatchedDateOrder.forEach(incidentID => {
    ...
    const firstEnRouteUnit = _findFirst(incidentUnitRecords, 'Dispatched Date', 'En Route Date')
    ...
    const travelTimes = _extractTimes(incidentUnitRecords, 'En Route Date', 'Arrival Date')
    const toSceneTimes = _extractTimes(incidentUnitRecords, 'Dispatched Date', 'Arrival Date')
    // let onSceneTime   = (baseRecord['Last Unit Cleared Date'] - firstArrivedUnit['Arrival Date'])/1000;
    const onSceneTimes = _extractTimes(incidentUnitRecords, 'Arrival Date', 'Clear Date')
    ...
    let personnelTimes = _.map(incidentRecords, (v) => (v['Clear Date'] - v['Dispatched Date']) / 1000)
    ...
    if (uniqPersonnel.length !== personnel.length) {
      ...
      _.each(uniqPersonnel, (p) => {
        ...
        const personnelRecords = _.filter(incidentRecords, (v) => v['User Login ID'] === p)
        const earliestDispatchDate = _.sortBy(personnelRecords, 'Dispatched Date')[0]['Dispatched Date']
        const latestClearDate = _.reverse(_.sortBy(personnelRecords, 'Clear Date'))[0]['Clear Date']
        const newPersonnelTime = (latestClearDate - earliestDispatchDate) / 1000
...
const _createStats = function (rawValues) {
  ...
  const typeStats = {}
  Object.assign(typeStats, defaultIncidentTypes, statsOutput.incident_stats.types, _.countBy(rawValues.incident_types))
  statsOutput.incident_stats.types = typeStats
  new Map([
    ['first_unit_reaction', _.chain(rawValues.reaction_times).flatten().compact().value()],
  ...
  const collapsedPersonnelTimes = _.map(rawValues.personnel_times, function (pt) {
    return sum(pt)
  })
  const incident = {
  ...
  statsOutput.personnel_stats.num_unique_responders = _.uniq(_.flatten(rawValues.personnel)).length
  ...
  statsOutput.apparatus_stats = {
    ...
    num_unique_used: _.without(_.uniq(_.flatten(rawValues.apparatus)), 'POV').length
  }
  ...
  rawValues.call_regions.forEach((region, i) => {
    ...
    if (!statsOutput.region_stats[region].incident_types) {
      ...
      Object.assign(statsOutput.region_stats[region].incident_types, defaultIncidentTypes)
    }
    statsOutput.region_stats[region].incident_types[callType] += 1
    statsOutput.region_stats[region].num_incidents += 1
    if (rawValues.travel_times[i]) rawRegions[region].push(rawValues.travel_times[i])
  })
  Object.keys(rawRegions).forEach(region => {
    ...
    const compactedArr = _.chain(rawRegions[region]).flatten().compact().value()
